...
function copyLegend(src, dst)
try
    sourceFig = ancestor(src, 'figure');
    legends = findobj(sourceFig, 'Type', 'legend');
    legends = legends(arrayfun(@(lgd) isOwnedByAxes(lgd, src), legends));
catch
    legends = [];
end
if ~isempty(legends) && strcmp(legends(1).Visible, 'on')
    try
        lgd = legend(dst, 'show', 'Interpreter', 'none');
        copyObjectProps(legends(1), lgd, {'Location','Position','Box','FontSize','FontName', ...
            'FontWeight','FontAngle','TextColor','Color','EdgeColor','Orientation', ...
            'NumColumns','Interpreter','Visible'});
    catch
    end
end
end
